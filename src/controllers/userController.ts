...
export const authenticateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const authUser = {uid:user.uid, username:user.username, roles:user.roles}
...
export const autoLoginUser = async(req: Request, res: Response) => {
  try {
    ...
    logger.info(`Auto-login successful for user: ${currentUser?.uid || "NULL"}`);
    res.status(200).json(currentUser);
  } catch (error) {
    logger.error(`Failed to auto-login user for userID ${ req.currentUser?.uid }:`, error);
