...
export const generateUserToken = (user: IUser) => {
  try {
    logger.info(`Generating token for user: ${user.uid}`);
    const token = jwt.sign({ userId: user.uid, _id: user._id }, env.JWT_SECRET, {
      expiresIn: "1d", // 1 day
    });
    logger.info(`Successfully generated token for user: ${user.uid}`);
    return token;
  } catch (error) {
    logger.error(`Failed to generate user token for piUID ${ user.uid }:`, error);
...
export const decodeUserToken = async (token: string) => {
  try {
    ...
    logger.info(`Successfully decoded token and found user: ${associatedUser.uid}`);
